...
EditTextOnScrollView::Ptr NewDefiFactory::create(Activity *publisher, const sf::Font &font, const sf::Vector2f &position, ScrollRectView *rectView, EventListener::EventCallback onTextEntered)
{
    const sf::Vector2f newDefiSize(890.f, 27.f);
    const unsigned int charSize = 20;
    EditTextOnScrollView::Ptr newDefiTextBox = std::make_unique<EditTextOnScrollView>(publisher, font, "New definition", charSize, position, newDefiSize, rectView);
